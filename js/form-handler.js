...
(function () {
  ...
  function mostrarErrores(errs) {
    ...
    document.querySelectorAll(".form-field-error").forEach(function (el) { try { el.classList.remove("form-field-error"); } catch (e) {} });
    errs.forEach(function (er) {
      if (!er || !er.campo) return;
      try { er.campo.classList.add("form-field-error"); } catch (e) {}
      ...
      try { er.campo.parentNode.appendChild(sp); } catch (e) {}
      setTimeout(function () { safeRemove(sp); try { er.campo.classList.remove("form-field-error"); } catch (e) {} }, 4000);
    });
    if (errs[0] && errs[0].campo) { try { errs[0].campo.scrollIntoView({ behavior: "smooth", block: "center" }); } catch (e) {} setTimeout(function () { try { errs[0].campo.focus(); } catch (e) {} }, 400); }
  }
  ...
  function mostrarExito(seccion, c) {
    ...
    try { (panel || seccion).appendChild(div); } catch (e) {}
    for (var k in c) { if (c[k]) { c[k].value = ""; try { c[k].dispatchEvent(new Event("change", { bubbles: true })); } catch (e) {} } }
